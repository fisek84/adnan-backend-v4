...
- 2025-12-24 – KANON-FIX-010_CICD_RELEASES – Kanonski CI/CD sloj: GitHub Actions `ci.yml` (pre-commit + pytest na PR/push main) + `release.yml` (tag `v*` + auto GitHub Release notes, VERSION/tag usklađivanje preko `system_version.py`) – (testovi: local: pre-commit PASS + pytest PASS; happy: `.\test_runner.ps1` – ALL HAPPY PATH TESTS PASSED; GitHub Actions: CI green; Release green; release tag: `v1.0.4`)
2025-12-24 – KANON-FIX-011_PHASE12_WARNINGS_CLEANUP – Pydantic V2 warnings cleanup + pytest anyio backend stabilizacija + httpx test deprecations cleanup – (testovi: pre-commit PASS; pytest PASS; happy: .\test_happy_path.ps1 PASS)