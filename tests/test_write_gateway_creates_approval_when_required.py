...
def test_write_requires_approval_creates_approval_and_no_side_effect(
...
def test_commit_write_blocks_when_approval_pending(monkeypatch: pytest.MonkeyPatch):
    ...
    class _FakeApprovals:
        ...
        def is_fully_approved(self, approval_id: str) -> bool:
            return (
                approval_id in self._m
                and self._m[approval_id].get("status") == "approved"
            )

    import services.approval_state_service as ass

    monkeypatch.setattr(
        ass, "get_approval_state", lambda: _FakeApprovals(), raising=True
    )
...
def test_commit_write_blocks_when_approval_rejected_or_invalid(
    monkeypatch: pytest.MonkeyPatch,
):
    from services.write_gateway.write_gateway import PolicyDecision, WriteGateway
...
def test_commit_write_allows_only_when_approval_approved(
    monkeypatch: pytest.MonkeyPatch,
):
    from services.write_gateway.write_gateway import PolicyDecision, WriteGateway
