...
def test_arm_persists_reload_and_fail_closed(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
	...
	monkeypatch.setattr(nas, "get", lambda *_a, **_k: (_ for _ in ()).throw(RuntimeError("boom")), raising=True)
	r8 = client.post(
		"/api/notion-ops/bulk/create",
		headers={"Authorization": f"Bearer {tok_admin}"},
		json={"items": []},
