...
def test_audit_emits_for_auth_denied_401(monkeypatch: pytest.MonkeyPatch):
    ...
    assert (
        e is not None
    ), f"expected auth_denied event, got {[x.event_type for x in events]}"
    assert e.request_id == rid
    assert e.route == "/api/ai-ops/approval/approve"


def test_audit_emits_for_execute_start_raw_refresh_snapshot(
    monkeypatch: pytest.MonkeyPatch,
):
    ...
    monkeypatch.setattr(
        gateway_server, "_require_boot_services", _boot_fail, raising=True
    )
    ...
    assert (
        e is not None
    ), f"expected execute_start event, got {[x.event_type for x in events]}"
    assert e.request_id == rid
...
def test_audit_emits_for_approval_approved(monkeypatch: pytest.MonkeyPatch):
    ...
    assert (
        e is not None
    ), f"expected approval_approved event, got {[x.event_type for x in events]}"
    assert e.request_id == rid
...
def test_audit_emits_for_execute_resume_unit(monkeypatch: pytest.MonkeyPatch):
    ...
    orch._execute_after_approval = _stub_execute_after_approval.__get__(
    ...
    assert (
        e is not None
    ), f"expected execute_resume event, got {[x.event_type for x in events]}"
