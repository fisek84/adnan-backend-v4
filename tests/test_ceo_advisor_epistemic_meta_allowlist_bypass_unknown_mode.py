...
@pytest.mark.parametrize(
    ...
    [
        "kako ti to znas ?",
        "na osnovu cega to znas?",
    ],
)
def test_epistemic_meta_questions_bypass_unknown_mode_when_general_knowledge_disabled(
    monkeypatch,
    user_text: str,
):
    from services.ceo_advisor_agent import create_ceo_advisor_agent

    # Production hard mode: no general knowledge, offline-safe.
    ...
    # Must contain deterministic epistemic explanation.
    assert "znam samo iz" in txt
    assert "ovog razgovora" in txt

    tr = out.trace or {}
    assert tr.get("intent") == "assistant_epistemic"
