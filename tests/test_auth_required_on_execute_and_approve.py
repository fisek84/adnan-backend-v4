...
def test_non_chat_routes_require_auth_and_chat_stays_public(
    monkeypatch: pytest.MonkeyPatch,
):
...
def test_require_principal_returns_401_for_missing_and_invalid_auth(
    monkeypatch: pytest.MonkeyPatch,
):
...
def test_require_role_returns_403_for_valid_principal_without_required_role(
    monkeypatch: pytest.MonkeyPatch,
):
    client = _dependency_test_client()
    tok = _token_for(
...
def test_require_scope_returns_403_for_valid_principal_without_required_scope(
    monkeypatch: pytest.MonkeyPatch,
):
    client = _dependency_test_client()
    tok = _token_for(
...
def test_execute_endpoints_require_auth_and_privileged_role(
    monkeypatch: pytest.MonkeyPatch,
):
    client = _client_without_boot(monkeypatch)
    secret = "test-secret"
    ...
    for path, body in (
        ...
        r2 = client.post(
            path, headers={"Authorization": f"Bearer {user_tok}"}, json=body
        )
        assert r2.status_code == 403


def test_execute_and_preview_remain_functional_for_privileged_principal(
    monkeypatch: pytest.MonkeyPatch,
):
    from gateway import gateway_server
