...
def _token_for(
    monkeypatch: pytest.MonkeyPatch, *, sub: str, roles: list[str], secret: str
) -> str:
...
def test_request_without_x_request_id_gets_generated_and_propagated(
...
def test_request_with_existing_x_request_id_is_echoed_and_state_matches(
    monkeypatch: pytest.MonkeyPatch,
):
    client = _client_with_probe(monkeypatch)
    tok = _token_for(monkeypatch, sub="admin-1", roles=["admin"], secret="test-secret")
