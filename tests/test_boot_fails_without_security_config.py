...
def test_boot_fails_fast_when_jwt_config_missing(monkeypatch: pytest.MonkeyPatch, tmp_path):
	...
	monkeypatch.setattr(gateway_server, "validate_runtime_env_or_raise", lambda: None, raising=True)
...
def test_boot_fails_fast_when_rbac_policy_missing_or_empty(monkeypatch: pytest.MonkeyPatch, tmp_path):
	monkeypatch.chdir(tmp_path)
	_set_production_mode(monkeypatch)
