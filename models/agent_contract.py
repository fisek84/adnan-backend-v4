...
class ProposedCommand(_AllowExtraBaseModel):
    ...
    reason: Optional[str] = Field(default=None, description="Zašto se predlaže ova komanda.")
    dry_run: bool = Field(default=True, description="Uvijek True za /api/chat.")
    requires_approval: bool = Field(default=True, description="Da li bi u write toku tražilo approval.")
...
class AgentInput(_AllowExtraBaseModel):
    ...
    preferred_agent_id: Optional[str] = Field(
        default=None,
        description="Opcionalno: klijent može eksplicitno tražiti agenta."
    )
...
class AgentOutput(_AllowExtraBaseModel):
    ...
    trace: Dict[str, Any] = Field(default_factory=dict, description="Trace metadata (routing, scoring, itd.).")
