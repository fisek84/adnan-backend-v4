...
class AgentRouterService:
    ...
    async def route(self, agent_input: AgentInput) -> AgentOutput:
        ...
        # Defense-in-depth: enforce canonical invariants on output.
        out.read_only = True
...
def _normalize_ascii(s: str) -> str:
    # minimal “latin” normalize for intent matching
    repl = {
        "š": "s",
        "đ": "d",
        "č": "c",
...
def _parse_filter(text: str, key: str) -> Optional[str]:
    """
    Robust parsing:
      status:Aktivan limit:20            -> Aktivan
      status=Aktivan priority:Visok      -> Aktivan
      status aktivan limit 20            -> aktivan

    Stops at next known key token or end of string.
    """
    t = (text or "").strip()
    if not t:
        return None
...
def ceo_clone_agent(agent_input: AgentInput, ctx: Dict[str, Any]) -> AgentOutput:
    ...
    if any(x in lower_ascii for x in ["ko si ti", "tko si ti", "who are you", "ko si"]):
        text = (
            f"{who} (CEO Advisor, READ-only): Ja sam kontrolisani CEO advisory sloj "
            "unutar Adnan.AI OS-a. Ne radim direktne write operacije; koristim snapshot "
            "(identity/memory/knowledge/Notion) i mogu vratiti analizu + prijedloge komandi."
        )
        return AgentOutput(
            text=text,
            proposed_commands=[],
    ...
    text = (
        f"{who} (CEO Advisor, READ-only): Razumio sam poruku. Mogu odgovoriti na osnovu snapshot-a.\n\n"
        "Primjeri:\n"
        "- inventory\n"
...
def specialist_notion_agent(agent_input: AgentInput, ctx: Dict[str, Any]) -> AgentOutput:
    ...
    text = (
        "Notion Specialist (read-only): Mogu mapirati zahtjev na Notion operacije i vratiti prijedloge komandi. "
